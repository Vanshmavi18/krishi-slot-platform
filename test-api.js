...
async function run() {
  console.log('--- Testing AgriQueue Backend Endpoints ---');

  // 1. Health
  const resHealth = await fetch(`${BASE}/api/health`).then(r => r.json());
  console.log('1. Health Check:', resHealth.status);

  // 2. Request OTP via Email
  const resEmailOtp = await fetch(`${BASE}/api/auth/send-email-otp`, {
    method: 'POST',
  ...
  const resVerify = await fetch(`${BASE}/api/auth/verify-email-otp`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ email: 'ramesh.farmer@agriqueue.in', otp: otpToVerify, savePassword: 'farmerPass2026' })
  }).then(r => r.json());
  ...
  const resPwdLogin = await fetch(`${BASE}/api/auth/login-email-password`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ email: 'ramesh.farmer@agriqueue.in', password: 'farmerPass2026' })
  }).then(r => r.json());
  ...
  const resCheck = await fetch(`${BASE}/api/auth/check-email?email=ramesh.farmer@agriqueue.in`).then(r => r.json());
  console.log('3c. Email Auth Check:', resCheck.exists, 'Has Password:', resCheck.hasPassword);

  // 4. Slots availability
  const resSlots = await fetch(`${BASE}/api/slots/availability?centreId=CTR-UP-01&date=2026-09-12`).then(r => r.json());
  console.log('4. Slot Windows Available:', resSlots.slots?.length);

  // 5. Book a Slot
  const resBook = await fetch(`${BASE}/api/slots/book`, {
    method: 'POST',
  ...
  // 6. Queue status
  const resQueue = await fetch(`${BASE}/api/queue/status?centreId=CTR-UP-01&token=A-047`).then(r => r.json());
  ...
  const resSms = await fetch(`${BASE}/api/sms/logs?phone=9876543210`).then(r => r.json());
  ...
  const resAdmin = await fetch(`${BASE}/api/auth/login-staff`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ staffId: 'admin@agriqueue.in', password: 'admin123' })
  }).then(r => r.json());
  ...
  const resBuyer = await fetch(`${BASE}/api/auth/login-buyer`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  ...
  const resNotifs = await fetch(`${BASE}/api/notifications?userId=` + activeUser.id).then(r => r.json());
  ...
  const resMarket = await fetch(`${BASE}/api/buyer/marketplace`).then(r => r.json());
  console.log('11. Buyer Marketplace Lots:', resMarket.lots?.length);

  const resOrder = await fetch(`${BASE}/api/buyer/orders`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      buyerId: 'BUYER-01',
